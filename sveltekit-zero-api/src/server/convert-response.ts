...
export function convertResponse<T>(response: T | KitResponse, options: ZeroAPIServerOptions = {}): T | Response {
	...
	const { client: {} = {}, sever: { stringify = true, logWithCause = 'non-ok' } = {} } = options

	// @ts-expect-error Hidden property
	const cause = response.cause as unknown // TODO instance of error - then include stack
	if (cause && (logWithCause === 'all' || (logWithCause === 'non-ok' && !response.ok))) {
		...
		const responseMsg = (response as unknown as { message: string }).message
